...
def update_user(user_id: int, user: pydantic_models.UserToUpdate = fastapi.Body(), current_user: Admin = Depends(get_current_user)): # используя fastapi.Body() мы явно указываем, что отправляем информацию в теле запроса
    """Обновляем юзера"""
    if user_id == user.id:
...
@crud.db_session
def create_transaction(user_id, transaction: pydantic_models.TransactionToCreate, current_user: Admin = Depends(get_current_user)):
