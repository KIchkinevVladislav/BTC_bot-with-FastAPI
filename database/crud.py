...
def create_wallet(
    ...
    """
    Cоздание кошелька пользователя
    :param user:
    :param private_key:
    :param testnet: в тестовой сети ли мы работаем или нет
...
def create_user(
    ...
    :param tg_id: обязательный параметр (id в Телеграм)
    :param nick:
    :return: User object
    """
    if nick:
...
def create_transaction(
    ...
    if amount_btc_without_fee + fee > sender.wallet.balance:
        return f'Too low balance: {sender.wallet.balance}'  # проверяем хватает ли средств в кошельки для отправки
...
def update_all_wallets():
    """
    Обновление состояние всех кошельков
    При приходе по базе кошельков печает адрес и баланс каждого
    :return:
    """
...
def get_user_by_id(id: int):
    """
    Получаев пользователя
    :param id: id объекта пользователя в базе данных
    :return: User object
    """
    return User[id]
...
def get_transaction_info(transaction: pydantic_models.Transaction):
    """
    Получаем словарь с атрибутами объекта транзакции
...
def get_wallet_info(wallet: pydantic_models.Wallet):
    """
    Получаем словарь с атрибутами объекта кошелька
...
def get_user_info(user: pydantic_models.User):
    """
    Получаем словарь с атрибутами объекта Пользователя
...
def update_user(user: pydantic_models.UserToUpdate):
    """
    Обновляем данные в модели пользователя
    :param user:
    :return:
    """
